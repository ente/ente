...
| 3 | 3.1 | Publish the baseline release and replace the owner's old app | M | 🟡 working | Museum, PostgreSQL, MinIO, and the private HTTPS routes passed health checks. Published audited baseline `1.3.59` (`2158`) to `trusted-testers` after adding the owner privately. Firebase CLI returned exit code 0 but omitted release references, so the guarded publisher preserved a read-only partial-attempt record; the owner then confirmed in the Firebase console that the expected release and group assignment exist, and the upload was not retried. Paused before device cutover because the Android device is unavailable. Resume by accepting the Firebase invitation, confirming old-package cloud backup and account recovery material, uninstalling `com.vanton1.ente.photos.selfhosted`, installing `me.vanton.ente.photos.selfhosted` through Firebase, signing in, and verifying foreground encrypted upload/download without migrating app-local state. |
