...
|------:|----:|-------|:----:|--------|-------|
| 1 | 1.1 | Rename the self-hosted Android application ID | M | ⚪ not started | Change only the `selfhosted` flavor to release ID `me.vanton.ente.photos.selfhosted`; retain the shared `io.ente.photos` namespace, normal `.debug` suffix, iOS identity, signing inputs, and official Android flavors. Update current assertions and build documentation without rewriting historical evidence. |
| 1 | 1.2 | Build, sign, and audit the replacement APK | S | ⚪ not started | Produce a configurable release with the existing signing identity and verify its package, version, build type, SDK range, application binary interfaces, compiled default HTTPS origin, archive integrity, certificate fingerprint, and SHA-256. Keep the working old package installed until this replacement exists. |
