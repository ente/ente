...
| 3 | 3.1 | Publish the audited iOS baseline through Firebase | M | 🔴 blocked / needs decision | Prepare fresh `1.3.59` build `2160` from the pushed source and publish that immutable pair as the first guarded Firebase baseline. Private/server health gates pass and Task 2.6 resolves the clean-checkout defect; resumption now requires committing and pushing the reviewed fix so the guarded preparer can prove its critical script matches the release commit. No IPA, manifest, Firebase release, receipt, or installed application changed. |
