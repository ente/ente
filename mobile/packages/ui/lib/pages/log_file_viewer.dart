...
class _LogFileViewerState extends State<LogFileViewer> {
  ...
  Future<void> _refresh() async {
    ...
    try {
      ...
      await _loadOlder();
      if (_isSearching) await _loadAllEntries();
    } catch (_) {
  ...
  Future<void> _loadOlder() {
    if (_loadFailed || !_reader.hasMore) return Future.value();
    final activeLoad = _activeLoad;
  ...
  Future<void> _loadAllEntries() async {
    while (mounted && _isSearching && !_loadFailed && _reader.hasMore) {
      await _loadOlder();
    }
  }
  ...
  void _toggleSearch() {
    ...
    if (_isSearching) _loadAllEntries();
  }
  ...
  Future<List<EntePopupMenuOption<int>>> _sessionOptions() async {
    while (mounted && !_loadFailed && _reader.hasMore) {
      await _loadOlder();
    }
    if (!mounted) return [];
    return [
