...
use crate::emoji::reaction_emojis;
...
pub(crate) fn canonical_reaction(emoji: &str) -> Result<&'static str> {
    static REACTIONS: LazyLock<HashMap<String, &'static str>> = LazyLock::new(|| {
        reaction_emojis()
            .iter()
            .flat_map(|entry| {
                std::iter::once(entry.emoji.as_str())
                    .chain(entry.skins.iter().map(|skin| skin.emoji.as_str()))
            })
            .map(|emoji| (key(emoji), emoji))
            .collect()
    });
...
mod tests {
    ...
    #[test]
    fn every_picker_entry_is_unique_and_can_be_sent() {
        let mut seen = std::collections::HashSet::new();
        for entry in reaction_emojis() {
            for emoji in std::iter::once(entry.emoji.as_str())
                .chain(entry.skins.iter().map(|skin| skin.emoji.as_str()))
            {
                assert!(seen.insert(key(emoji)), "duplicate {emoji}");
                assert_eq!(canonical_reaction(emoji).unwrap(), emoji);
                assert_eq!(canonical_reaction(&key(emoji)).unwrap(), emoji);
            }
        }
        assert_eq!(seen.len(), 3944);
    }

    #[test]
    fn accepts_fully_qualified_sequences_and_optional_emoji_selectors() {
        for emoji in ["🫱🏻‍🫲🏿", "👩🏽‍💻", "🇮🇳", "1️⃣", "👨‍👩‍👧‍👦"]
        {
            assert_eq!(canonical_reaction(emoji).unwrap(), emoji);
            assert_eq!(canonical_reaction(&key(emoji)).unwrap(), emoji);
