...
impl SearchState {
    fn search_graph(&self) -> &Graph {
        self.graph.as_ref().unwrap_or(&PENDING_SEARCH_GRAPH)
    }
}
...
impl VecDb {
    ...
    pub fn open_cost(path: &Path) -> OpenCost {
        let key = registry_key_for(path).unwrap_or_else(|_| path.to_path_buf());
        if registry_holds_live(&key) {
    ...
    pub fn open_read_only(
        ...
        if let Some(slot) = existing_path_slot(&resolved) {
            ...
            match observed {
                Some(shared) if !shared.is_closed() => {
                    return join_live(shared, dims, storage, true);
                }
                Some(closing) => {
                    drop(closing);
        ...
        Ok(Self {
            shared: Arc::new(build_read_only(&resolved, dims, storage)?),
            read_only: true,
        })
    }
    ...
    pub fn add_with_attrs(
        ...
        if add_changes_nothing(&self.shared.state_read(), key, payload, attrs) {
            return Ok(());
        }
    ...
    fn bulk_add_internal(
        ...
        let changed: Vec<usize> = {
            ...
            (0..keys.len())
                .filter(|&index| last_entry_of_key[keys[index].as_str()] == index)
                .filter(|&index| {
                    !add_changes_nothing(&st, &keys[index], payload_of(index), attrs_of(index))
                })
                .collect()
        };
        if changed.is_empty() {
        ...
        let records: Vec<LogEntry<'_>> = changed
            .iter()
            .map(|&index| LogEntry::Add {
                key: &keys[index],
        ...
        apply_policy(&self.shared, &mut half, changed.len(), now)
    }
...
fn registry_holds_live(key: &Path) -> bool {
    let Some(slot) = existing_path_slot(key) else {
        return false;
    };
    match slot.try_lock() {
...
fn open_cost_from_files(path: &Path) -> OpenCost {
    ...
    drop(file);
    let Ok(generation) = header_generation(&header) else {
        return OpenCost::NeedsRebuild;
    };
...
fn add_changes_nothing(
    ...
    state
        .arena
        .slot_holding(key, vector)
        .is_some_and(|slot| attrs_match(state.attrs.get(slot), attrs))
}

fn attrs_match(stored: Option<&[Attribute]>, attrs: &[Attribute]) -> bool {
    let stored = stored.unwrap_or_default();
    stored.len() == attrs.len()
        && stored.iter().all(|held| {
            let mut named = attrs.iter().filter(|attr| attr.name == held.name);
            named
                .next()
                .is_some_and(|attr| attr_values_match(&attr.value, &held.value))
                && named.next().is_none()
        })
}

fn attr_values_match(left: &AttrValue, right: &AttrValue) -> bool {
    match (left, right) {
        (AttrValue::F64(left), AttrValue::F64(right)) => left.to_bits() == right.to_bits(),
        _ => left == right,
    }
}

fn apply_add_record(
    arena: &mut VectorArena,
    graph: &mut Option<Graph>,
    attr_table: &mut AttrTable,
    key: &str,
    vector: VectorPayload<'_>,
    attrs: &[Attribute],
) -> Result<(), VecDbError> {
    match arena.upsert_payload(key, vector)? {
        UpsertOutcome::Kept(slot) => attr_table.set(slot, attrs),
        UpsertOutcome::Appended { slot, retired } => {
            if let Some(retired) = retired {
                attr_table.clear(retired);
            }
            attr_table.set(slot, attrs);
...
fn apply_add(
    ...
    apply_add_record(arena, graph, attr_table, key, vector, attrs)?;
    *total_records += 1;
    Ok(())
}
...
fn restore_writer_mode(
    ...
    let restored = (|| {
        ...
        let mut log = Log::open(file, &shared.path, shared.dims, shared.storage)?;
...
fn build_writer(
    ...
    let mut log = if std::fs::metadata(path).is_ok() {
        ...
        match Log::create(path, dims, storage) {
            Ok(created) => created,
...
fn install_graph(shared: &Shared, graph: Graph) {
    let mut installable = Some(graph);
    loop {
...
fn build_read_only(path: &Path, dims: usize, storage: StorageKind) -> Result<Shared, VecDbError> {
    let file = match File::open(path) {
        Ok(file) => file,
        Err(error) if error.kind() == ErrorKind::NotFound => {
            return empty_read_only(path, dims, storage);
        }
    ...
    let mut log = Log::open(file, path, dims, storage)?;
...
fn empty_read_only(path: &Path, dims: usize, storage: StorageKind) -> Result<Shared, VecDbError> {
    Ok(read_only_shared(
        path,
        VectorArena::with_storage(dims, storage)?,
        Graph::new(),
        AttrTable::default(),
        0,
        0,
...
fn approximate_memory_bytes(state: &SearchState) -> usize {
    ...
    vector_bytes + key_bytes + graph_bytes + state.attrs.memory_bytes()
}
...
mod tests {
    ...
    use super::super::graph::GraphNodeParts;
    ...
    fn omitted_storage_requires_i8_for_creation_and_every_reopen() {
        ...
        let reader = VecDb::open_read_only(&path, 32, None).unwrap();
        assert_eq!(reader.stats().unwrap().storage, StorageKind::I8);
        assert!(!path.exists());
        drop(reader);
        let db = VecDb::open(&path, 32, None).unwrap();
        ...
        let db = VecDb::open(&f32_path, 32, Some(StorageKind::F32)).unwrap();
        let reject = || {
            let before = fs::read(&f32_path).unwrap();
            for open in [VecDb::open, VecDb::open_read_only] {
                assert!(matches!(
                    open(&f32_path, 32, None),
    ...
    fn explicit_configuration_is_verified_on_live_and_disk_opens() {
        ...
        for storage in [StorageKind::F32, StorageKind::I8] {
            let path = dir.path().join(storage.to_string());
    ...
    fn missing_and_incomplete_indexes_use_the_explicit_configuration() {
        ...
        for storage in [StorageKind::F32, StorageKind::I8] {
            for incomplete in [false, true] {
                let path = dir.path().join(format!("{storage}-{incomplete}"));
                ...
                drop(reader);
                let writer = VecDb::open(&path, dims, Some(storage)).unwrap();
        ...
        for open in [VecDb::open, VecDb::open_read_only] {
            let path = dir.path().join("invalid");
            assert!(matches!(
                open(&path, 8, Some(StorageKind::I8)),
                Err(VecDbError::InvalidDimensions {
                    dims: 8,
                    storage: StorageKind::I8
                })
    ...
    fn cosine_scores_unnormalized_vectors_in_every_search_path() {
        let dir = TempDir::new().unwrap();
        for storage in [StorageKind::F32, StorageKind::I8] {
            let db = VecDb::open(&dir.path().join(storage.to_string()), 32, Some(storage)).unwrap();
            let vector = |x, y| {
    ...
    #[test]
    fn cosine_scores_survive_replacement_compaction_snapshot_replay_and_reset() {
        let dir = TempDir::new().unwrap();
        for storage in [StorageKind::F32, StorageKind::I8] {
            let path = dir.path().join(storage.to_string());
            let db = VecDb::open(&path, 32, Some(storage)).unwrap();
            ...
            vector[1] = 1.0;
            db.add("kept", &vector).unwrap();
            let mut transient = vec![0.0; 32];
            transient[2] = 1.0;
            db.add("transient", &transient).unwrap();
            ...
            let assert_score = |db: &VecDb| {
                for exact in [false, true] {
                    ...
                    assert!(found[0].distance.abs() < 1.0e-5);
                }
            };
            assert_score(&db);
            drop(db);
            for snapshot in [true, false] {
                ...
                assert_score(&VecDb::open_read_only(&path, 32, Some(storage)).unwrap());
                assert_score(&VecDb::open(&path, 32, Some(storage)).unwrap());
            }
            let db = VecDb::open(&path, 32, Some(storage)).unwrap();
            db.reset().unwrap();
            db.add("kept", &vector).unwrap();
    ...
    fn cosine_f32_handles_extreme_finite_magnitudes() {
        let dir = TempDir::new().unwrap();
        let db = VecDb::open(&dir.path().join("db"), 8, Some(StorageKind::F32)).unwrap();
    ...
    fn open_writer(path: &Path) -> VecDb {
        VecDb::open(path, DIMS, Some(StorageKind::F32)).unwrap()
    }
    ...
    fn assert_corrupt_open_preserves(path: &Path) {
        ...
        assert!(matches!(
            VecDb::open(path, DIMS, Some(StorageKind::F32)),
            Err(VecDbError::Corrupt(_))
        ));
        assert!(matches!(
            VecDb::open_read_only(path, DIMS, Some(StorageKind::F32)),
            Err(VecDbError::Corrupt(_))
        ));
    ...
    fn read_only_replay_adopts_records_a_foreign_writer_appended_after_capture() {
        ...
        let mut captured_for_read_only =
            Log::open(File::open(&path).unwrap(), &path, DIMS, StorageKind::F32).unwrap();
    ...
    fn writer_lock_contention_preserves_incomplete_logs() {
        ...
        for name in [&path, &alias] {
            assert!(matches!(
                VecDb::open(name, DIMS, Some(StorageKind::F32)),
                Err(VecDbError::Locked(_))
            ));
        }
    ...
    fn dotted_and_plain_paths_share_one_instance() {
        ...
        let second = VecDb::open(&dotted, DIMS, Some(StorageKind::F32)).unwrap();
        assert!(Arc::ptr_eq(&first.shared, &second.shared));
    ...
    fn open_racing_a_parked_close_always_gets_a_usable_handle() {
        for _ in 0..5 {
            ...
            let opener = {
                let path = path.clone();
                thread::spawn(move || VecDb::open(&path, DIMS, Some(StorageKind::F32)))
            };
    ...
    fn promote_compacted_log_replaces_the_log_durably() {
        ...
        let log = open_existing_writer_log(&path, DIMS, StorageKind::F32).unwrap();
    ...
    fn restore_writer_mode_replays_writes_made_while_the_log_was_closed() {
        ...
        let checkpoint = log.checkpoint();
        drop(log);
        let mut foreign = open_existing_writer_log(&path, DIMS, StorageKind::F32).unwrap();
        foreign
            .append(&[LogEntry::Add {
                key: "foreign",
    ...
    fn concurrent_bulk_add_with_live_readers_and_flush() {
        ...
        for reader_index in 0..READERS {
            let handle = VecDb::open(&path, DIMS, Some(StorageKind::F32)).unwrap();
    ...
    fn symlinked_paths_share_one_instance_and_one_lock() {
        ...
        let via_alias = VecDb::open(&alias, DIMS, Some(StorageKind::F32)).unwrap();
        via_path
    ...
    #[test]
    fn open_cost_reports_ready_for_a_live_instance() {
        let dir = TempDir::new().unwrap();
        let path = dir.path().join("db");
        let db = open_writer(&path);
        bulk_add(&db, &bulk_entries(0, 5, 980)).unwrap();
        assert_eq!(VecDb::open_cost(&path), OpenCost::Ready);
        drop(db);
        assert_eq!(VecDb::open_cost(&path), OpenCost::NeedsRebuild);
    }

    #[test]
    fn open_cost_walks_every_file_branch() {
        let dir = TempDir::new().unwrap();
        let path = dir.path().join("db");
        assert_eq!(VecDb::open_cost(&path), OpenCost::Absent);
        fs::write(&path, [0x45u8, 0x56, 0x44, 0x42, 0x01]).unwrap();
        assert_eq!(VecDb::open_cost(&path), OpenCost::Absent);
        fs::remove_file(&path).unwrap();
        drop(open_writer(&path));
        assert_eq!(VecDb::open_cost(&path), OpenCost::Ready);
        let db = open_writer(&path);
        bulk_add(&db, &bulk_entries(0, 4, 985)).unwrap();
        drop(db);
        assert_eq!(VecDb::open_cost(&path), OpenCost::NeedsRebuild);
        drop(open_writer(&path));
        assert!(snapshot_exists(&path));
        assert_eq!(VecDb::open_cost(&path), OpenCost::Ready);
        let healthy_snapshot = fs::read(snapshot_path(&path)).unwrap();
        let mut corrupted = healthy_snapshot.clone();
        corrupted[20] ^= 0x5A;
        fs::write(snapshot_path(&path), &corrupted).unwrap();
        assert_eq!(VecDb::open_cost(&path), OpenCost::NeedsRebuild);
        fs::remove_file(&path).unwrap();
        fs::remove_file(snapshot_path(&path)).unwrap();
        let db = open_writer(&path);
        db.add("fresh", &seeded_unit_vector(1, DIMS)).unwrap();
        drop(db);
        fs::write(snapshot_path(&path), &healthy_snapshot).unwrap();
        assert_eq!(VecDb::open_cost(&path), OpenCost::NeedsRebuild);
    }

    #[test]
    fn open_cost_flags_a_snapshot_covering_beyond_the_log() {
        let dir = TempDir::new().unwrap();
        let path = dir.path().join("db");
        let db = open_writer(&path);
        db.add("first", &seeded_unit_vector(1, DIMS)).unwrap();
    ...
    #[test]
    fn open_cost_ignores_the_writer_lock_and_leaves_a_live_writer_undisturbed() {
        let dir = TempDir::new().unwrap();
        let path = dir.path().join("db");
        let db = open_writer(&path);
        bulk_add(&db, &bulk_entries(0, 3, 990)).unwrap();
        db.flush().unwrap();
        drop(db);
        let held = WriterLock::acquire(&path).unwrap();
        assert_eq!(VecDb::open_cost(&path), OpenCost::Ready);
    ...
    #[test]
    fn searches_serve_exact_results_while_the_graph_rebuild_is_pending() {
        const COUNT: usize = 2000;
        let dir = TempDir::new().unwrap();
        let path = dir.path().join("db");
        let control_path = dir.path().join("control");
        let entries = bulk_entries(0, COUNT, 8000);
        let replacement = seeded_unit_vector(31_415, DIMS);
        let query = seeded_unit_vector(4321, DIMS);
        let bulk_queries: Vec<Vec<f32>> = (0..3)
            .map(|index| seeded_unit_vector(5000 + index, DIMS))
            .collect();
        let stored_keys = vec![
            "key-10".to_string(),
            "missing".to_string(),
        ...
        bulk_add(&control, &entries).unwrap();
        let expected_single = control.search(&query, &exact).unwrap();
        let expected_bulk = control.bulk_search(&bulk_queries, &exact).unwrap();
        let expected_stored = control
            .bulk_search_stored(&stored_keys, 5, None, true, false)
            .unwrap();
        control.add("key-0", &replacement).unwrap();
        let expected_exact_after = control.search(&query, &exact).unwrap();
        let expected_approx_after = control.search(&query, &approx).unwrap();
        drop(control);
        {
            let db = open_writer(&path);
        ...
        let opener = {
            let path = path.clone();
            thread::spawn(move || VecDb::open(&path, DIMS, Some(StorageKind::F32)).unwrap())
        };
        let key = registry_key_for(&path).unwrap();
        let deadline = Instant::now() + Duration::from_secs(60);
        let joined = loop {
            ...
            if registered {
                break VecDb::open_read_only(&path, DIMS, Some(StorageKind::F32)).unwrap();
            }
            thread::yield_now();
        };
        let pin = joined.shared.state_read();
        assert!(pin.graph.is_none());
        assert_eq!(joined.search(&query, &approx).unwrap(), expected_single);
        assert_eq!(joined.search(&query, &exact).unwrap(), expected_single);
        assert_eq!(
            joined.bulk_search(&bulk_queries, &approx).unwrap(),
            expected_bulk
        );
        assert_eq!(
            joined
                .bulk_search_stored(&stored_keys, 5, None, false, false)
                .unwrap(),
            expected_stored
        );
        assert!(!opener.is_finished());
        let writer = VecDb::open(&path, DIMS, Some(StorageKind::F32)).unwrap();
        assert!(Arc::ptr_eq(&writer.shared, &joined.shared));
        let adder = {
            let vector = replacement.clone();
            thread::spawn(move || {
                writer.add("key-0", &vector).unwrap();
                writer
            })
        };
        ...
        assert_eq!(opened.search(&query, &exact).unwrap(), expected_exact_after);
        assert_eq!(
            opened.search(&query, &approx).unwrap(),
            expected_approx_after
        );
        drop(opened);
        drop(writer);
        drop(joined);
        ...
        assert_eq!(
            reopened.search(&query, &approx).unwrap(),
            expected_approx_after
        );
    }
    ...
    #[test]
    fn attrs_do_not_leak_across_removes_replacements_and_appended_slots() {
        let dir = TempDir::new().unwrap();
        let path = dir.path().join("db");
        let db = open_writer(&path);
        ...
        assert_eq!(db.shared.state_read().arena.slot_of_key("c"), Some(2));
        assert_eq!(db.get_attrs("c").unwrap(), None);
        assert!(db.remove("b").unwrap());
        db.add_with_attrs("d", &vector, &sample_attrs(12)).unwrap();
        assert_eq!(db.shared.state_read().arena.slot_of_key("d"), Some(3));
        assert_eq!(db.get_attrs("d").unwrap().unwrap(), sample_attrs(12));
        db.add_with_attrs("d", &vector, &sample_attrs(13)).unwrap();
        {
            let st = db.shared.state_read();
            assert_eq!(st.arena.slot_of_key("d"), Some(3));
            assert_eq!(st.attrs.get(3).unwrap(), sample_attrs(13));
        }
        db.add_with_attrs("d", &seeded_unit_vector(4, DIMS), &sample_attrs(14))
            .unwrap();
        {
            let st = db.shared.state_read();
            assert_eq!(st.arena.slot_of_key("d"), Some(4));
            assert_eq!(st.attrs.get(3), None);
            assert_eq!(st.attrs.get(4).unwrap(), sample_attrs(14));
        }
        db.add("a", &vector).unwrap();
        assert_eq!(db.shared.state_read().arena.slot_of_key("a"), Some(5));
        assert_eq!(db.get_attrs("a").unwrap(), None);
        drop(db);
        let reopened = open_writer(&path);
        ...
        assert_eq!(reopened.get_attrs("d").unwrap().unwrap(), sample_attrs(14));
    }
    ...
    fn attrs_replay_identically_across_reopen_and_read_only() {
        ...
        db.add_with_attrs("appended", &vector, &sample_attrs(22))
            .unwrap();
        db.flush().unwrap();
        drop(db);
        let verify = |db: &VecDb| {
            ...
            assert_eq!(db.get_attrs("appended").unwrap().unwrap(), sample_attrs(22));
        };
        let read_only = VecDb::open_read_only(&path, DIMS, Some(StorageKind::F32)).unwrap();
        verify(&read_only);
        drop(read_only);
    ...
    fn attrs_survive_a_failed_compaction_and_its_retry() {
        ...
        verify(&db);
        fs::remove_dir(temp_sibling(&path)).unwrap();
        let rewrite = |range: std::ops::Range<usize>, seed: u64| {
            let (keys, vectors): (Vec<String>, Vec<Vec<f32>>) =
                bulk_entries(range.start, range.len(), seed)
                    .iter()
                    .cloned()
                    .unzip();
    ...
    fn open_i8(path: &Path) -> VecDb {
        VecDb::open(path, I8_DIMS, Some(StorageKind::I8)).unwrap()
    }
    ...
    fn logged_add_records(path: &Path, dims: usize) -> Vec<(String, StoredVector)> {
        let mut log = Log::open(File::open(path).unwrap(), path, dims, StorageKind::I8).unwrap();
    ...
    fn existing_index_reports_storage_mismatch_before_its_dimensions_are_judged() {
        ...
        let live = VecDb::open(&narrow, 8, Some(StorageKind::F32)).unwrap();
        live.add("n", &seeded_unit_vector(1, 8)).unwrap();
        assert!(matches!(
            VecDb::open(&narrow, 8, Some(StorageKind::I8)),
            Err(VecDbError::StorageMismatch {
                expected: StorageKind::I8,
                actual: StorageKind::F32
            })
        ...
        drop(VecDb::open(&wide, I8_DIMS, Some(StorageKind::F32)).unwrap());
        assert!(matches!(
            VecDb::open(&wide, 8, Some(StorageKind::I8)),
            Err(VecDbError::StorageMismatch {
                expected: StorageKind::I8,
                actual: StorageKind::F32
            })
        ...
        assert!(matches!(
            VecDb::open(&wide, 8, Some(StorageKind::F32)),
            Err(VecDbError::DimensionMismatch {
                expected: 8,
                actual: I8_DIMS
            })
        ...
        assert!(matches!(
            VecDb::open_read_only(&dir.path().join("i8"), 8, Some(StorageKind::I8)),
            Err(VecDbError::DimensionMismatch {
                expected: 8,
                actual: I8_DIMS
            })
        ...
        assert!(matches!(
            VecDb::open(&dir.path().join("fresh"), 8, Some(StorageKind::I8)),
            Err(VecDbError::InvalidDimensions {
                dims: 8,
                storage: StorageKind::I8
            })
    ...
    fn storage_kind_is_verified_on_every_open_path() {
        ...
        let f32_db = VecDb::open(&f32_path, I8_DIMS, Some(StorageKind::F32)).unwrap();
        f32_db.add("f", &seeded_unit_vector(1, I8_DIMS)).unwrap();
        assert_eq!(f32_db.stats().unwrap().storage, StorageKind::F32);
        assert!(matches!(
            VecDb::open(&f32_path, I8_DIMS, Some(StorageKind::I8)),
            Err(VecDbError::StorageMismatch {
                expected: StorageKind::I8,
                actual: StorageKind::F32
            })
        ));
        let verified = VecDb::open(&f32_path, I8_DIMS, Some(StorageKind::F32)).unwrap();
        ...
        assert_eq!(
            VecDb::open(&f32_path, I8_DIMS, Some(StorageKind::F32))
                .unwrap()
                .stats()
                .unwrap()
                .storage,
        ...
        assert!(VecDb::open(&dir.path().join("narrow"), 16, Some(StorageKind::F32)).is_ok());
        assert_eq!(
            VecDb::open_read_only(&dir.path().join("absent"), I8_DIMS, Some(StorageKind::I8))
                .unwrap()
                .stats()
                .unwrap()
                .storage,
    ...
    fn i8_index_survives_compaction_replay_and_snapshot_with_identical_bytes_and_results() {
        ...
        let read_only = VecDb::open_read_only(&path, I8_DIMS, Some(StorageKind::I8)).unwrap();
        verify(&read_only);
        drop(read_only);
        let with_snapshot = VecDb::open(&path, I8_DIMS, Some(StorageKind::I8)).unwrap();
        verify(&with_snapshot);
        drop(with_snapshot);
        fs::remove_file(snapshot_path(&path)).unwrap();
        let rebuilt = VecDb::open(&path, I8_DIMS, Some(StorageKind::I8)).unwrap();
        verify(&rebuilt);
    ...
    fn i8_interrupted_sessions_build_the_same_graph_as_one_uninterrupted_session() {
        ...
        {
            let db = VecDb::open(&split_path, I8_DIMS, Some(StorageKind::I8)).unwrap();
            assert_eq!(db.stats().unwrap().storage, StorageKind::I8);
            db.add("tail-new", &tail_new).unwrap();
            db.add("key-3", &older_upsert).unwrap();
        }
        let split = VecDb::open(&split_path, I8_DIMS, Some(StorageKind::I8)).unwrap();
    ...
    fn i8_stats_report_the_storage_and_a_quarter_of_the_vector_memory() {
        let dir = TempDir::new().unwrap();
        let dims = 512;
        let f32_db = VecDb::open(&dir.path().join("f32"), dims, Some(StorageKind::F32)).unwrap();
        let i8_db = VecDb::open(&dir.path().join("i8"), dims, Some(StorageKind::I8)).unwrap();
    ...
    fn unfindable(db: &VecDb, vectors: &[Vec<f32>], probes: &[usize]) -> Vec<usize> {
        probes
            ...
            .copied()
            .filter(|&index| {
                db.search(&vectors[index], &limit_params(1)).unwrap()[0].key
                    != format!("key-{index}")
            })
            .collect()
    }

    fn unfindable_exactly(db: &VecDb, vectors: &[Vec<f32>], probes: &[usize]) -> Vec<usize> {
        let params = SearchParams {
            limit: Some(1),
            exact: true,
            ..SearchParams::default()
    ...
    fn fresh_rebuild_of(path: &Path, rebuilt_path: &Path) -> VecDb {
        fs::copy(path, rebuilt_path).unwrap();
        open_writer(rebuilt_path)
    }

    fn slot_map(db: &VecDb) -> Vec<(String, u32)> {
        let st = db.shared.state_read();
    ...
    fn level_zero_sources(graph: &Graph, target: u32) -> Vec<u32> {
        graph
            .slots()
            .filter(|&slot| graph.neighbors_of(slot, 0).contains(&target))
            .collect()
    }

    fn level_lists(graph: &Graph, slot: u32) -> Vec<Vec<u32>> {
    ...
    #[test]
    fn a_replacement_appends_a_slot_and_leaves_the_retired_node_linked() {
        let dir = TempDir::new().unwrap();
        let path = dir.path().join("db");
        let db = open_writer(&path);
        let entries = bulk_entries(0, 300, 7700);
        bulk_add(&db, &entries).unwrap();
        let (retired, level, lists, sources) = {
            let st = db.shared.state_read();
            let graph = st.graph.as_ref().unwrap();
            let slot = st.arena.slot_of_key("key-7").unwrap();
            (
                slot,
                graph.level_of(slot).unwrap(),
                level_lists(graph, slot),
                level_zero_sources(graph, slot),
            )
        };
        assert!(!sources.is_empty());
        let old_vector = entries[7].1.clone();
        let new_vector = seeded_unit_vector(0x7C00_0000, DIMS);
        db.add("key-7", &new_vector).unwrap();
        {
            let st = db.shared.state_read();
            let graph = st.graph.as_ref().unwrap();
            assert_eq!(st.arena.slot_of_key("key-7"), Some(300));
            assert!(!st.arena.is_alive(retired));
            assert_eq!(st.arena.live_count(), 300);
            assert_eq!(st.arena.slot_count(), 301);
            assert_eq!(st.arena.dead_count(), 1);
            assert_eq!(graph.level_of(retired), Some(level));
        ...
        assert_eq!(
            db.search(&new_vector, &limit_params(1)).unwrap()[0].key,
            "key-7"
        );
        assert!(
            db.search(&old_vector, &limit_params(10))
                .unwrap()
                .iter()
                .all(|hit| hit.key != "key-7")
        );
    }

    #[test]
    fn dead_slots_are_never_recycled_and_compaction_reclaims_them() {
        let dir = TempDir::new().unwrap();
        let path = dir.path().join("db");
        let db = open_writer(&path);
        bulk_add(&db, &bulk_entries(0, 200, 7800)).unwrap();
        assert!(db.remove("key-3").unwrap());
        db.add("fresh", &seeded_unit_vector(0x7D00_0000, DIMS))
            .unwrap();
        assert_eq!(db.shared.state_read().arena.slot_of_key("fresh"), Some(200));
        db.add("key-3", &seeded_unit_vector(0x7D00_0001, DIMS))
            .unwrap();
        {
            let st = db.shared.state_read();
            assert_eq!(st.arena.slot_of_key("key-3"), Some(201));
            assert!(!st.arena.is_alive(3));
            assert_eq!(st.arena.live_count(), 201);
            assert_eq!(st.arena.slot_count(), 202);
        }
        compact(&db.shared, &mut db.shared.writer_half()).unwrap();
        {
            let st = db.shared.state_read();
            assert_eq!(st.arena.slot_count(), st.arena.live_count());
            assert_eq!(st.arena.dead_count(), 0);
            test_support::assert_graph_invariants(st.graph.as_ref().unwrap());
        }
        db.add("after", &seeded_unit_vector(0x7D00_0002, DIMS))
            .unwrap();
        assert_eq!(db.shared.state_read().arena.slot_of_key("after"), Some(201));
    }
    ...
    fn deleting_most_of_a_cluster_keeps_the_survivors_findable_after_compaction() {
        ...
        let rebuilt = fresh_rebuild_of(&path, &rebuilt_path);
    ...
    fn moving_most_of_a_cluster_elsewhere_keeps_the_survivors_findable() {
        ...
        for batch in 1..=4 {
            let destinations: Vec<Vec<f32>> = (0..2700)
                .step_by(10)
                .map(|index| vectors[index + batch].clone())
                .collect();
            db.bulk_add(&moved, &destinations).unwrap();
            let current = generation_of(&path);
            compacted |= current != generation;
            generation = current;
            if !compacted {
                assert_eq!(
                    unfindable(&db, &vectors, &survivors),
                    Vec::<usize>::new(),
                    "batch {batch} before any compaction"
                );
            }
        }
        assert!(compacted, "no compaction fired");
        let retained = unfindable(&db, &vectors, &survivors);
        drop(db);
        let rebuilt_path = dir.path().join("rebuilt");
        let rebuilt = fresh_rebuild_of(&path, &rebuilt_path);
        assert_eq!(retained, unfindable(&rebuilt, &vectors, &survivors));
        let reopened = open_writer(&path);
        assert_eq!(unfindable(&reopened, &vectors, &survivors), retained);
    }

    #[test]
    fn two_replacement_batches_keep_the_untouched_cluster_findable() {
        let dir = TempDir::new().unwrap();
        let path = dir.path().join("db");
        let (db, keys, vectors) = clustered_index(&path, 6300);
        let survivors: Vec<usize> = (2700..3000).step_by(10).collect();
        ...
        assert!(compacted, "no compaction fired");
        assert_eq!(
            unfindable_exactly(&db, &vectors, &survivors),
            Vec::<usize>::new()
        );
        let retained = unfindable(&db, &vectors, &survivors);
        db.flush().unwrap();
    ...
    #[test]
    fn a_reopened_log_reproduces_the_slot_numbering_the_writer_assigned() {
        for seed in 0..6u64 {
            let dir = TempDir::new().unwrap();
            let path = dir.path().join("db");
            let db = open_writer(&path);
            ...
            for _ in 0..600 {
                ...
                match roll % 6 {
                    ...
                    1 => {
                        let key = live.remove(roll as usize % live.len());
            ...
            let slots_now = slot_map(&db);
            db.flush().unwrap();
            drop(db);
            fs::remove_file(snapshot_path(&path)).unwrap();
            let replayed = open_writer(&path);
            assert_eq!(slots_now, slot_map(&replayed), "seed {seed} renumbered");
        }
    }
    ...
    fn assert_snapshot_round_trips(db: &VecDb) {
        ...
        test_support::assert_identical_graphs(&reloaded, graph);
    }

    fn churn_attrs(state: &mut u64) -> Vec<Attribute> {
        match splitmix64(state) % 3 {
            0 => Vec::new(),
            1 => sample_attrs((splitmix64(state) % 5) as i64),
            _ => {
                let mut attrs = sample_attrs((splitmix64(state) % 5) as i64);
                attrs.reverse();
                attrs
            }
        }
    ...
    fn property_churn_run(seed: u64, rounds: usize, initial: usize) -> (usize, usize) {
        ...
        let mut live: HashMap<String, (Vec<f32>, Vec<Attribute>)> = (0..initial)
            .map(|index| {
                (
                    format!("key-{index}"),
                    (seeded_unit_vector(seed + index as u64, DIMS), Vec::new()),
                )
            })
            .collect();
        let mut removed: Vec<String> = Vec::new();
        ...
        for _ in 0..rounds {
            ...
            match roll % 12 {
                0..=3 => {
                    let key = format!("new-{next_id}");
                    next_id += 1;
                    let attrs = churn_attrs(&mut state);
                    db.add_with_attrs(&key, &vector, &attrs).unwrap();
                    live.insert(key, (vector, attrs));
                }
                4..=5 => {
                    if !keys.is_empty() {
                ...
                _ => {
                    ...
                    let mut batch: Vec<(String, Vec<f32>, Vec<Attribute>)> = (0..count)
                        .map(|offset| {
                            (
                                format!("new-{}", next_id + offset as u64),
                                seeded_unit_vector(splitmix64(&mut state), DIMS),
                                churn_attrs(&mut state),
                            )
                        })
                        .collect();
                    ...
                    if let Some(key) = keys.first() {
                        let (held_vector, held_attrs) = live[key].clone();
                        batch.push((key.clone(), held_vector, held_attrs));
                    }
                    let batch_keys: Vec<String> =
                        batch.iter().map(|(key, _, _)| key.clone()).collect();
                    let batch_vectors: Vec<Vec<f32>> =
                        batch.iter().map(|(_, vector, _)| vector.clone()).collect();
                    let batch_attrs: Vec<Option<Vec<Attribute>>> = batch
                        .iter()
                        .map(|(_, _, attrs)| Some(attrs.clone()))
                        .collect();
                    db.bulk_add_with_attrs(&batch_keys, &batch_vectors, &batch_attrs)
                        .unwrap();
                    for (key, vector, attrs) in batch {
                        live.insert(key, (vector, attrs));
                    }
                }
            }
        ...
        for (key, (vector, attrs)) in &live {
            assert!(db.contains(key).unwrap(), "{key} vanished");
            assert!(
                attrs_match(db.get_attrs(key).unwrap().as_deref(), attrs),
                "{key} lost the attributes it was last given"
            );
            let matches = db.search(vector, &limit_params(1)).unwrap();
        ...
        let expected = probe
            .as_ref()
            .map(|vector| db.search(vector, &limit_params(5)).unwrap())
            .unwrap_or_default();
        drop(db);
        let reopened = open_writer(&path);
        assert_eq!(reopened.len().unwrap(), live.len());
        for (key, (_, attrs)) in &live {
            assert!(
                attrs_match(reopened.get_attrs(key).unwrap().as_deref(), attrs),
                "{key} lost its attributes across a reopen"
            );
        }
    ...
    fn threshold_params() -> SearchParams {
        SearchParams {
    ...
    fn open_storage(path: &Path, storage: StorageKind) -> VecDb {
        VecDb::open(path, I8_DIMS, Some(storage)).unwrap()
    }

    fn logged_records(path: &Path, dims: usize, storage: StorageKind) -> Vec<LogRecord> {
        let mut log = Log::open(File::open(path).unwrap(), path, dims, storage).unwrap();
        let mut scanner = log.scan().unwrap();
        let mut records = Vec::new();
        while let Some((record, _)) = scanner.next_record().unwrap() {
            records.push(record);
    ...
    fn graph_node_count(db: &VecDb) -> usize {
        db.shared
            .state_read()
            .graph
            .as_ref()
    ...
    #[test]
    fn identical_re_adds_keep_the_key_inside_the_threshold_on_every_reader() {
        for storage in [StorageKind::F32, StorageKind::I8] {
            let dir = TempDir::new().unwrap();
            let path = dir.path().join("db");
            ...
            let keys: Vec<String> = (0..3000).map(|index| format!("key-{index}")).collect();
            let vectors: Vec<Vec<f32>> = (0..3000)
                .map(|index| seeded_unit_vector(0xE000_0000 + index as u64, I8_DIMS))
                .collect();
            db.bulk_add(&keys, &vectors).unwrap();
            let expected = db.search(&vectors[7], &threshold_params()).unwrap();
            assert_eq!(expected[0].key, "key-7");
            let slots = db.shared.state_read().arena.slot_count();
            let nodes = graph_node_count(&db);
            let log_bytes = db.stats().unwrap().log_bytes;
            for _ in 0..33 {
                db.add("key-7", &vectors[7]).unwrap();
            }
            let stats = db.stats().unwrap();
            assert_eq!(stats.dead_count, 0, "{storage} counted a dead record");
            assert_eq!(stats.log_bytes, log_bytes, "{storage} appended a record");
            assert_eq!(db.shared.state_read().arena.slot_count(), slots);
            assert_eq!(graph_node_count(&db), nodes);
            assert_eq!(
                db.search(&vectors[7], &threshold_params()).unwrap(),
                expected
            );
            db.flush().unwrap();
            drop(db);
            ...
            assert_eq!(
                read_only.search(&vectors[7], &threshold_params()).unwrap(),
                expected
            );
            drop(read_only);
            let reopened = open_storage(&path, storage);
            assert_eq!(
                reopened.search(&vectors[7], &threshold_params()).unwrap(),
                expected
            );
            assert_eq!(reopened.stats().unwrap().dead_count, 0);
            assert_eq!(reopened.shared.state_read().arena.slot_count(), slots);
            drop(reopened);
            let rebuilt_path = dir.path().join("rebuilt");
            fs::copy(&path, &rebuilt_path).unwrap();
            let rebuilt = open_storage(&rebuilt_path, storage);
            assert_eq!(
                rebuilt.search(&vectors[7], &threshold_params()).unwrap(),
                expected
            );
    ...
    #[test]
    fn an_i8_re_add_that_quantizes_to_the_stored_bytes_is_a_noop() {
        let dir = TempDir::new().unwrap();
        let path = dir.path().join("db");
        let db = open_i8(&path);
        let entries = i8_entries(0, 200, 0xE100_0000);
        bulk_add(&db, &entries).unwrap();
        let original = entries[7].1.clone();
        ...
        let jittered: Vec<f32> = original
            ...
            .collect();
        assert_ne!(jittered, original);
        assert_eq!(
            StoredVector::quantize(&jittered),
            StoredVector::quantize(&original)
        );
        let log_bytes = db.stats().unwrap().log_bytes;
        db.add("key-7", &jittered).unwrap();
        let unchanged = db.stats().unwrap();
        assert_eq!(unchanged.dead_count, 0);
        assert_eq!(unchanged.log_bytes, log_bytes);
        assert_eq!(db.shared.state_read().arena.slot_count(), 200);
        assert_eq!(db.shared.state_read().arena.slot_of_key("key-7"), Some(7));
        let moved: Vec<f32> = original.iter().map(|value| value + 1.0e-3).collect();
        assert_ne!(
            StoredVector::quantize(&moved),
            StoredVector::quantize(&original)
        );
        db.add("key-7", &moved).unwrap();
        let replaced = db.stats().unwrap();
        assert_eq!(replaced.dead_count, 1);
        assert!(replaced.log_bytes > log_bytes);
        let st = db.shared.state_read();
        assert_eq!(st.arena.slot_count(), 201);
        assert_eq!(st.arena.slot_of_key("key-7"), Some(200));
        assert!(!st.arena.is_alive(7));
    }

    #[test]
    fn an_f32_re_add_is_compared_bit_for_bit() {
        let dir = TempDir::new().unwrap();
        let path = dir.path().join("db");
        let db = open_writer(&path);
        let entries = bulk_entries(0, 100, 0xE200_0000);
        bulk_add(&db, &entries).unwrap();
        let log_bytes = db.stats().unwrap().log_bytes;
        db.add("key-7", &entries[7].1).unwrap();
        let unchanged = db.stats().unwrap();
        assert_eq!(unchanged.dead_count, 0);
        assert_eq!(unchanged.log_bytes, log_bytes);
        let mut one_bit = entries[7].1.clone();
        one_bit[3] = f32::from_bits(one_bit[3].to_bits() ^ 1);
        db.add("key-7", &one_bit).unwrap();
        assert_eq!(db.stats().unwrap().dead_count, 1);
        assert_eq!(db.shared.state_read().arena.slot_of_key("key-7"), Some(100));
        let mut positive_zero = one_bit.clone();
        positive_zero[4] = 0.0;
        db.add("key-7", &positive_zero).unwrap();
        assert_eq!(db.stats().unwrap().dead_count, 2);
        let mut negative_zero = positive_zero.clone();
        negative_zero[4] = -0.0;
        db.add("key-7", &negative_zero).unwrap();
        assert_eq!(db.stats().unwrap().dead_count, 3);
        assert_eq!(db.shared.state_read().arena.slot_of_key("key-7"), Some(102));
        assert_eq!(
            db.get("key-7").unwrap().unwrap()[4].to_bits(),
            (-0.0f32).to_bits()
        );
    }

    #[test]
    fn an_attrs_only_change_keeps_the_slot_and_its_node() {
        let dir = TempDir::new().unwrap();
        let path = dir.path().join("db");
        let db = open_writer(&path);
        let entries = bulk_entries(0, 60, 0xE300_0000);
        bulk_add(&db, &entries).unwrap();
        let nodes = graph_node_count(&db);
        let records = logged_records(&path, DIMS, StorageKind::F32).len();
        db.add_with_attrs("key-7", &entries[7].1, &sample_attrs(3))
            .unwrap();
        assert_eq!(
            logged_records(&path, DIMS, StorageKind::F32).len(),
            records + 1
        );
        assert_eq!(graph_node_count(&db), nodes);
        {
            let st = db.shared.state_read();
            assert_eq!(st.arena.slot_of_key("key-7"), Some(7));
            assert_eq!(st.arena.slot_count(), 60);
        }
        assert_eq!(db.stats().unwrap().dead_count, 1);
        assert_eq!(db.get_attrs("key-7").unwrap().unwrap(), sample_attrs(3));
        let mut reordered = sample_attrs(3);
        reordered.reverse();
        let log_bytes = db.stats().unwrap().log_bytes;
        db.add_with_attrs("key-7", &entries[7].1, &reordered)
            .unwrap();
        assert_eq!(db.stats().unwrap().log_bytes, log_bytes);
        assert_eq!(db.get_attrs("key-7").unwrap().unwrap(), sample_attrs(3));
        ...
        assert_eq!(
            rebuilt.get_attrs("key-7").unwrap().unwrap(),
            sample_attrs(3)
        );
        assert_eq!(rebuilt.shared.state_read().arena.slot_count(), 60);
    }

    #[test]
    fn a_bulk_add_appends_only_the_entries_that_change() {
        let dir = TempDir::new().unwrap();
        let path = dir.path().join("db");
        let db = open_writer(&path);
        let entries = bulk_entries(0, 40, 0xE400_0000);
        bulk_add(&db, &entries).unwrap();
        db.add_with_attrs("key-1", &entries[1].1, &sample_attrs(1))
            .unwrap();
        db.add_with_attrs("key-2", &entries[2].1, &sample_attrs(9))
            .unwrap();
        let records = logged_records(&path, DIMS, StorageKind::F32).len();
        let mut unchanged_attrs = sample_attrs(1);
        unchanged_attrs.reverse();
        let moved = seeded_unit_vector(0xE400_0FFF, DIMS);
        let fresh = seeded_unit_vector(0xE400_0EEE, DIMS);
        let keys = vec![
            "key-0".to_string(),
            "key-1".to_string(),
            "key-1".to_string(),
        ...
        let expected_map = attack_slot_map(&db);
        {
            let st = db.shared.state_read();
            assert_eq!(st.arena.slot_of_key("key-0"), Some(0));
            assert_eq!(st.arena.slot_of_key("key-1"), Some(1));
            assert_eq!(st.arena.slot_of_key("key-2"), Some(40));
            assert_eq!(st.arena.slot_of_key("new-key"), Some(41));
            assert_eq!(st.arena.slot_count(), 42);
            assert_eq!(st.attrs.get(2), None);
        }
        assert_eq!(db.get_attrs("key-1").unwrap().unwrap(), sample_attrs(2));
        assert_eq!(db.get_attrs("new-key").unwrap().unwrap(), sample_attrs(4));
        assert_eq!(db.get("key-2").unwrap().unwrap(), moved);
        let log_bytes = db.stats().unwrap().log_bytes;
        let mut settled_attrs = sample_attrs(2);
        settled_attrs.reverse();
        db.bulk_add_with_attrs(&keys[..2], &vectors[..2], &[None, Some(settled_attrs)])
            .unwrap();
        assert_eq!(db.stats().unwrap().log_bytes, log_bytes);
        db.flush().unwrap();
        drop(db);
        ...
        fs::copy(&path, &rebuilt_path).unwrap();
        let rebuilt = open_writer(&rebuilt_path);
        assert_eq!(attack_slot_map(&rebuilt), expected_map);
        assert_eq!(
            rebuilt.get_attrs("key-1").unwrap().unwrap(),
            sample_attrs(2)
        );
        assert_eq!(
            rebuilt.get_attrs("new-key").unwrap().unwrap(),
            sample_attrs(4)
        );
        assert_eq!(rebuilt.shared.state_read().arena.slot_count(), 42);
    }

    #[test]
    fn unchanged_re_adds_do_not_advance_the_snapshot_cadence_or_the_policy() {
        let dir = TempDir::new().unwrap();
        let path = dir.path().join("db");
        let db = open_writer(&path);
        let entries = bulk_entries(0, 50, 0xE500_0000);
        bulk_add(&db, &entries).unwrap();
        let generation = generation_of(&path);
        let log_bytes = db.stats().unwrap().log_bytes;
        set_mutations(&db, SNAPSHOT_HARD_CAP - 1);
        for round in 0..2000 {
            let (key, vector) = &entries[round % entries.len()];
            db.add(key, vector).unwrap();
        }
        assert_eq!(mutations(&db), SNAPSHOT_HARD_CAP - 1);
        assert!(!snapshot_exists(&path));
        assert_eq!(generation_of(&path), generation);
        let stats = db.stats().unwrap();
        assert_eq!(stats.dead_count, 0);
        assert_eq!(stats.log_bytes, log_bytes);
        assert_eq!(stats.records_since_snapshot, SNAPSHOT_HARD_CAP - 1);
        assert_eq!(db.shared.state_read().arena.slot_count(), 50);
    }

    #[test]
    fn an_attribute_float_is_compared_bit_for_bit() {
        let dir = TempDir::new().unwrap();
        let path = dir.path().join("db");
        let db = open_writer(&path);
        let vector = seeded_unit_vector(8800, DIMS);
        let score = |value: f64| {
            vec![Attribute {
        ...
        db.add_with_attrs("key", &vector, &score(0.0)).unwrap();
        let records_before = db.stats().unwrap().dead_count;
        db.add_with_attrs("key", &vector, &score(0.0)).unwrap();
        assert_eq!(db.stats().unwrap().dead_count, records_before);
        db.add_with_attrs("key", &vector, &score(-0.0)).unwrap();
        assert_eq!(db.stats().unwrap().dead_count, records_before + 1);
        let stored = db.get_attrs("key").unwrap().unwrap();
        assert!(
            matches!(stored[0].value, AttrValue::F64(value) if value.to_bits() == (-0.0f64).to_bits())
        );
    }

    #[test]
    fn a_key_repeated_in_one_batch_keeps_its_last_entry() {
        let dir = TempDir::new().unwrap();
        let path = dir.path().join("db");
        let db = open_writer(&path);
        let entries = bulk_entries(0, 40, 0xE700_0000);
        bulk_add(&db, &entries).unwrap();
        db.add_with_attrs("key-3", &entries[3].1, &sample_attrs(1))
            .unwrap();
        let moved = seeded_unit_vector(0xE700_0FFF, DIMS);
        db.bulk_add_with_attrs(
            &["key-3".to_string(), "key-3".to_string()],
            &[moved, entries[3].1.clone()],
            &[Some(sample_attrs(2)), Some(sample_attrs(1))],
        )
        .unwrap();
        assert_eq!(db.get("key-3").unwrap().unwrap(), entries[3].1);
        assert_eq!(db.get_attrs("key-3").unwrap().unwrap(), sample_attrs(1));
    }

    #[test]
    fn a_removed_key_re_added_with_its_old_vector_gets_a_new_slot() {
        let dir = TempDir::new().unwrap();
        let path = dir.path().join("db");
        let db = open_writer(&path);
        let entries = bulk_entries(0, 40, 0xE600_0000);
        bulk_add(&db, &entries).unwrap();
        assert!(db.remove("key-7").unwrap());
        db.add("key-7", &entries[7].1).unwrap();
        {
            let st = db.shared.state_read();
            assert_eq!(st.arena.slot_of_key("key-7"), Some(40));
            assert!(!st.arena.is_alive(7));
            assert_eq!(st.arena.slot_count(), 41);
            assert_eq!(st.arena.live_count(), 40);
        }
        assert_eq!(db.stats().unwrap().dead_count, 2);
        assert_eq!(db.get("key-7").unwrap().unwrap(), entries[7].1);
        assert_eq!(
            db.search(&entries[7].1, &limit_params(1)).unwrap()[0].key,
            "key-7"
        );
    }
