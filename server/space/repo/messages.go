...
func (r *MessagesRepository) SetLikeWithChanged(ctx context.Context, messageID string, actorSpaceID string, like bool) (bool, error) {
	if like {
		...
		err := r.DB.QueryRowContext(ctx, `
			WITH previous AS (
				SELECT recipient_liked_at, recipient_reacted_at
				FROM space_messages
			...
			SET recipient_liked_at = COALESCE(m.recipient_liked_at, m.recipient_reacted_at, now_utc_micro_seconds()),
			...
			RETURNING previous.recipient_liked_at IS NULL AND previous.recipient_reacted_at IS NULL
...
func (r *MessagesRepository) SetReaction(ctx context.Context, messageID, actorSpaceID string, cipher, senderKey, recipientKey []byte) (bool, error) {
	...
	err := r.DB.QueryRowContext(ctx, `
		...
		SET recipient_reaction_cipher = $3,
		    ...
		    recipient_reacted_at = COALESCE(m.recipient_reacted_at, m.recipient_liked_at, now_utc_micro_seconds()),
