...
const EmojiPicker = React.lazy(async () => {
    const [{ default: Picker }, { reactionEmojis }] = await Promise.all([
        import("components/EmojiPicker"),
        import("ente-space-wasm"),
    ]);
    const catalog = await reactionEmojis();
    return {
        default: function LoadedEmojiPicker(
            props: Omit<React.ComponentProps<typeof Picker>, "catalog">,
        ) {
            return <Picker {...props} catalog={catalog} />;
        },
    };
});
